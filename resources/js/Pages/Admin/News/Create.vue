<template>
    <AdminLayout>
        <div class="mb-8 flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-slate-100">
            <div>
                <h1 class="text-2xl font-bold text-[#003366]">Add News</h1>
                <p class="text-sm text-slate-500">Create a new blog post or update</p>
            </div>
        ...
        <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="space-y-8">
                <div class="bg-white p-8 rounded-xl shadow-sm border border-slate-100">
                    ...
                    <div class="space-y-4">
                        ...
                        <button @click.prevent="showGallery = true" type="button" class="w-full py-4 bg-[#003366] text-white rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-black transition-all shadow-xl shadow-blue-900/10 active:scale-95 flex items-center justify-center gap-2">
                            <Plus class="w-4 h-4" /> Select From Gallery
                        </button>
                    </div>
                </div>
                ...
                <div class="bg-white p-8 rounded-xl shadow-sm border border-slate-100">
                    ...
                    <button type="submit" :disabled="form.processing" class="w-full mt-8 bg-[#FF6600] text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-orange-600 transition-all shadow-xl shadow-orange-900/20 disabled:opacity-50 active:scale-95">
                        Publish News
                    </button>
                </div>
            </div>
