  ...
  <StoreLayout>
    ...
    <section class="bg-white py-6 md:py-10 border-b border-slate-100">
      <div class="max-w-[1400px] mx-auto px-6 md:px-12 xl:px-16">
        <div class="grid lg:grid-cols-3 gap-5">
          ...
          >
            ...
            >
              ...
              >
                <div class="flex-1">
                  <span class="text-emerald-600 text-[11px] font-bold uppercase tracking-wider block mb-3">
                    New Arrivals
                  </span>
                  <h1 class="text-3xl md:text-4xl font-extrabold text-slate-900 leading-tight mb-5">
            ...
            >
              <button
                ...
                :key="i"
                @click="goToSlide(i)"
                :class="currentSlide === i ? 'w-6 bg-emerald-600' : 'w-2 bg-slate-400 hover:bg-slate-500'"
          ...
          >
            <span class="text-emerald-700 text-[10px] font-bold uppercase tracking-wider mb-3">Build your own</span>
            <h3 class="text-2xl md:text-3xl font-extrabold text-emerald-950 leading-tight mb-4">
              Custom PC<br/>Builder
            </h3>
            <p class="text-emerald-800 text-sm mb-6 max-w-[200px] font-medium">
              Pick your components.
            </p>
            <span class="px-5 py-2.5 bg-white text-emerald-700 border border-emerald-200 rounded-xl text-sm font-bold flex items-center gap-2 group-hover:bg-emerald-600 group-hover:border-emerald-600 group-hover:text-white transition-all duration-300 shadow-sm">
        ...
        <!-- Marquee strip -->
        <div class="mt-5 bg-white border border-slate-200 rounded-xl px-5 py-3 flex items-center gap-3 overflow-hidden shadow-sm">
          <Truck class="w-4 h-4 text-emerald-600 shrink-0" />
          <p class="text-xs text-slate-500 font-medium whitespace-nowrap overflow-hidden text-ellipsis">
            Nationwide delivery · Guaranteed Quality · Contact Center: 
            <span class="text-slate-700 font-bold tracking-wide">{{ settings.footer_phone || '01322-893290' }}</span>
          </p>
        </div>
      </div>
    </section>
    ...
    <section class="py-12 md:py-16 bg-slate-50 border-b border-slate-100">
      <div class="max-w-[1400px] mx-auto px-6 md:px-12 xl:px-16">
        ...
        >
          ...
          >
            <div class="w-16 h-16 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center p-3 overflow-hidden group-hover:bg-emerald-50 transition-colors duration-300">
              <img
                :src="cat.image || `https://placehold.co/200x200/f8fafc/64748b?text=${encodeURIComponent(cat.name)}`"
                :alt="cat.name"
                class="w-full h-full object-contain mix-blend-multiply"
              />
            </div>
            <span class="text-xs font-bold text-slate-700 text-center group-hover:text-emerald-700 transition-colors line-clamp-1">
              {{ cat.name }}
            </span>
          </Link>
        </div>
    ...
    <section class="py-12 md:py-16 bg-white border-b border-slate-100">
      <div class="max-w-[1400px] mx-auto px-6 md:px-12 xl:px-16">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
          <div class="flex gap-1 bg-slate-50 border border-slate-200 rounded-xl p-1 w-fit shadow-sm">
            <button
          ...
          <div class="flex gap-1.5">
            <button
              @click="scrollContainer('prod-container', -400)"
              class="p-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 transition-colors text-slate-500 shadow-sm"
            >
              <ChevronLeft class="w-4 h-4" />
            </button>
        ...
        <Transition name="tab-fade" mode="out-in">
          <div
            :key="activeTab"
            id="prod-container"
            class="grid grid-flow-col auto-cols-[calc((100%-1rem)/2)] sm:auto-cols-[calc((100%-2rem)/3)] lg:grid-cols-5 lg:auto-cols-auto gap-4 overflow-x-auto snap-x scroll-smooth no-scrollbar pb-3 pt-1"
          >
            <div
    ...
    <!-- BUNDLES -->
    <section v-if="combos?.length" class="py-12 md:py-16 bg-slate-50 border-b border-slate-100">
      <div class="max-w-[1400px] mx-auto px-6 md:px-12 xl:px-16">
        <div class="mb-8">
          <h2 class="text-xl md:text-2xl font-bold text-slate-900">
            Combo Deals
          </h2>
          <p class="text-slate-500 text-sm mt-1">Pre-configured bundles from our experts.</p>
        </div>
    ...
    <!-- BRANDS -->
    <section v-if="brands?.length" class="py-12 bg-white border-b border-slate-100">
      <div class="max-w-[1400px] mx-auto px-6 md:px-12 xl:px-16">
        <div class="flex justify-between items-center mb-6">
          <h3 class="text-xs font-bold text-slate-500 uppercase tracking-wider">Top Brands</h3>
        </div>
        <div
          id="brand-container"
          class="flex items-center gap-12 overflow-x-auto py-2 no-scrollbar snap-x scroll-smooth"
        >
          <Link
            v-for="brand in brands"
            :key="brand.id"
            :href="`/shop?brand=${brand.slug}`"
            class="shrink-0 h-10 flex items-center opacity-70 hover:opacity-100 hover:scale-105 transition-all duration-300 snap-start mix-blend-multiply"
          >
            <img
              v-if="brand.image"
              :src="brand.image"
            ...
            <span
              v-else
              class="text-base font-bold text-slate-700 hover:text-emerald-600 transition-colors"
            >
              {{ brand.name }}
            </span>
          </Link>
...
const tabbedProducts = computed(() => {
  ...
  if (activeTab.value === 'deals') return props.discountedProducts.slice(0, 10);
  return (props.topSelling.length ? props.topSelling : props.latestProducts).slice(0, 10);
});

const selectedProduct = ref(null);
const openQuickView   = (product) => { selectedProduct.value = product; };
...
const goToSlide   = (i)  => { currentSlide.value = ((i % sliderImages.value.length) + sliderImages.value.length) % sliderImages.value.length; };
...
onMounted(() => {
  if (sliderImages.value.length > 1) {
    autoSlide = setInterval(() => { if (!isPaused) nextSlide(); }, 6000);
  }
});
